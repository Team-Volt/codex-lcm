...
- [ ] Add `tests/dag.test.ts` covering:
...
- [ ] Update `tests/mcp.test.ts` to expect `lcm_get_session_graph`.
- [ ] Update `tests/plugin-manifest.test.ts` to expect a `skills` manifest entry and `skills/lcm-recall/SKILL.md`.
...
- [ ] Extend SQLite schema with `turn_id` and `tool_use_id` columns on `events`.
- [ ] Add `graph_nodes` and `graph_edges` tables with typed node/edge fields, metadata JSON, and indexes.
- [ ] Add deterministic node IDs:
  - `session:<session_id>`
  - `turn:<session_id>:<turn_id>`
  - `event:<event_id>`
  - `checkpoint:<session_id>:<event_count>`
- [ ] Build graph edges during ingestion:
...
- [ ] Prevent cycles before inserting edges using a recursive CTE from prospective child to prospective parent.
- [ ] Keep all derived indexing inside a transaction after raw append. If indexing fails, raw append must remain durable.

## Task 3: Long-Session Retrieval

- [ ] Add row-level `searchEvents()` over FTS.
- [ ] Add `getSessionGraph(sessionId, limit)` returning bounded nodes and edges.
- [ ] Add paged `getSession(sessionId, { limit, cursor })` returning `next_cursor`.
- [ ] Update `packContext()` to prioritize matching events, graph neighbors, latest checkpoint nodes, then bounded recent tail.
- [ ] Keep raw-log fallback behavior usable when SQLite is unavailable.

## Task 4: MCP And Skill Nudge

- [ ] Add MCP tool `lcm_get_session_graph`.
- [ ] Add optional `limit` and `cursor` to `lcm_get_session`.
- [ ] Update MCP server instructions to tell Codex to use LCM on resume, after compaction, when prior context is likely relevant, and before answering memory-dependent questions.
- [ ] Add `skills/lcm-recall/SKILL.md` with specific tool-use guidance.
- [ ] Add `"skills": "./skills/"` to `.codex-plugin/plugin.json`.

## Task 5: Docs And Verification

- [ ] Update architecture/design docs for DAG tables, edge invariants, checkpoints, and long-session retrieval.
- [ ] Update README with new MCP tool and skill.
- [ ] Run:
  - `npm test`
  - `npm run smoke`
